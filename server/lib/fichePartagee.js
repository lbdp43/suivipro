...
export async function creerProspectDepuisFiche(fiche, { auteurId, commercialId, notes = [] }) {
  ...
  await db.query(
    ...
    `INSERT INTO prospects (id, nom_etablissement, type_etablissement, nom_contact, telephone, email, adresse, ville, code_postal, departement, secteur, latitude, longitude, etape_pipeline, tags, commercial_id, notes, date_creation, date_modification, score, source_url, siret, siren, raison_sociale, tva_intracom)
     VALUES ($1,$2,$3,$4,$5,$17,$6,$7,$8,$9,'',$10,$11,'partage','[]',$12,$13,$14,$14,$15,$16,$18,$19,$20,$21)`,
    [id, String(fiche.nom_etablissement || 'Établissement partagé (à renommer)').slice(0, 200), fiche.type_etablissement || 'autre', fiche.nom_contact || '', fiche.telephone || '',
      ...
      fiche.email || '', fiche.siret || '', fiche.siren || '', fiche.raison_sociale || '', fiche.tva_intracom || '']
  );

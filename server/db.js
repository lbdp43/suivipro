...
async function initDatabase(attempt = 1) {
  ...
  try {
    ...
    // Un commercial peut aussi faire de la prospection (deux casquettes).
    try {
    ...
    // Migration : modèles d'emails v3 (liqueurs). On complète les modèles de référence manquants
    // sans jamais effacer la table : les modèles créés par l'équipe restent en place.
    ...
    if (tplCheck.rows.length === 0) {
      ...
      for (const t of tplsV3) {
        await client.query(
          'INSERT INTO email_templates (id, nom, sujet, corps, type) VALUES ($1,$2,$3,$4,$5) ON CONFLICT (id) DO UPDATE SET nom = EXCLUDED.nom, sujet = EXCLUDED.sujet, corps = EXCLUDED.corps, type = EXCLUDED.type',
          t
        );
      }

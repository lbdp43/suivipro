...
import { prospectsAAppelerAutourDe, RAYON_APPELS_KM } from '../utils/voisinage';
...
export default function BoutonAppelerAutour({ points, compact = false, stopPropagation = true, className = '' }: Props) {
  ...
  const cliquer = (e: React.MouseEvent) => {
    ...
    lancer.prospects(aAppeler.map(p => p.id));
  };

  return (
    <button
      onClick={cliquer}
      title={`Lancer une session d'appel avec les ${aAppeler.length} prospects à moins de ${RAYON_APPELS_KM} km`}
    ...
    >
      ...
      {compact ? aAppeler.length : `Appeler autour (${aAppeler.length})`}
    </button>
  );
}
